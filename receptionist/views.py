...
def generate_daily_bookings(request):
    ...
    if request.method == 'POST':
        ...
        # for doctor in doctors:
        #     for date in get_dates_from_number_of_days(int(input_number_of_days)):
        #         for slot in doctor.doctor_time_slots.all():
        #             DailySlotBooking.objects.create(doctor=doctor, date=date, time_slot=slot)
...
#     else:
#         return [date.today() + timedelta(days=i) for i in range(number_of_days)]
...
# get next working days from today ; check if today is a working day ; if not, get next working day and generate bookings
...
def time_slots_default_day_doctor(request):
    ...
    # if selected_doctor:
    #     doctor_time_slots = doctor_time_slots.filter(doctor=selected_doctor.user)
...
def receptionist_profile(request):
    ...
    # receptionist_profile_form = EditReceptionistForm(instance=receptionist.receptionist)
